...
const toChargeAmount = (priceEGP: number): number => {
  if (env.PAYPAL_CURRENCY === "EGP") return priceEGP;
...
export const paymentService = {
   ...
   * us it had happened.
   */
  async createOrder(userId: string, userEmail: string, selection: SeatSelection) {
  ...
  async captureOrder(userId: string, orderId: string) {
    ...
    if (
      detail.amount.currency_code !== booking.currency ||
  ...
  async cancelOrder(userId: string, orderId: string) {
    ...
    if (!booking) {
      return { message: "Nothing to cancel" } as const;
    }
    ...
    return { message: "Booking cancelled" } as const;
  },
};
