...
const assertConfigured = (): void => {
  if (!env.paypalEnabled) {
    throw new ApiError(
      503,
      "Payments are not configured on this server. Set PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET.",
    );
  }
};
...
export const paypal = {
  isConfigured: (): boolean => env.paypalEnabled,
  ...
  createOrder: (params: {
    ...
    request<PayPalOrder>("/v2/checkout/orders", {
      method: "POST",
  ...
  captureOrder: (orderId: string): Promise<PayPalCapture> =>
    request<PayPalCapture>(`/v2/checkout/orders/${orderId}/capture`, {
      method: "POST",
      requestId: `capture-${orderId}`,
    }),

  getOrder: (orderId: string): Promise<PayPalCapture> =>
    request<PayPalCapture>(`/v2/checkout/orders/${orderId}`, { method: "GET" }),
};
