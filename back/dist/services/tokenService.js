...
export const tokenService = {
    ...
    async rotate(presentedToken) {
        ...
        if (stored.revokedAt) {
            logger.warn({ subject: stored.subject }, "Reuse of a rotated refresh token — revoking all sessions for this account");
    ...
    async revoke(presentedToken) {
        await RefreshToken.updateOne({ tokenHash: hashRefreshToken(presentedToken), revokedAt: null }, { revokedAt: new Date() }).exec();
    },
    /** Ends every session for an account — password change, or a detected replay. */
    async revokeAllForSubject(subject) {
        await RefreshToken.updateMany({ subject, revokedAt: null }, { revokedAt: new Date() }).exec();
    },
    refreshCookieMaxAge() {
