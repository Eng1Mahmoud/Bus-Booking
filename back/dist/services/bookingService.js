...
const reference = () => `TZK-${crypto.randomBytes(5).toString("hex").toUpperCase()}`;
export const bookingService = {
     ...
     * than a throw.
     */
    async appendToHistory(booking) {
        // Counter sales have no account to mirror onto.
        if (!booking.user)
            return;
        try {
            await User.updateOne({ _id: booking.user }, {
                $push: {
                    bookingsHistory: {
                        from: booking.from,
                        to: booking.to,
            ...
            logger.error({ err: error, reference: booking.reference }, "Paid booking could not be mirrored onto the user document");
        }
    },
