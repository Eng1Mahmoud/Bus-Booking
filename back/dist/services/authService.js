...
const codeExpiry = () => new Date(Date.now() + env.VERIFICATION_CODE_TTL_MINUTES * 60_000);
export const authService = {
    ...
    async register({ email, password, FName, LName }) {
        if (await User.exists({ email })) {
            return { exist: true, message: "user already exists" };
        }
     ...
     */
    async verifyEmail({ verificationCode, user }) {
        ...
        if (!codeMatches(verificationCode, pending.codeHash)) {
            pending.attempts += 1;
            await pending.save();
            return { verification: false, message: "كود التحقق غير صحيح" };
        }
        ...
        await pending.deleteOne();
        return { verification: true, message: "تم انشاء الحساب بنجاح" };
    },
    ...
    async resetPassword({ email, password, verificationCode }) {
        ...
        if (!user) {
            return { verification: false, message: "User not found" };
        }
        ...
        await tokenService.revokeAllForSubject(user.id);
        return { verification: true, message: "Password updated" };
    },
};
